...
function ControlsPanel({ sendMessage }) {
  ...
  const inputCls = 'w-full bg-gray-800/60 text-gray-200 border border-gray-700/50 rounded-md px-2.5 py-1.5 text-xs focus:outline-none focus:border-gray-500/50 transition-colors';
          ...
          <div className="p-3 pt-0 space-y-2">
            <div className="flex gap-2 items-end">
              ...
              <div className="w-16">
                ...
                <input type="text" value={recordedKey ? recordedKey.toUpperCase() : ''} readOnly placeholder={recording === 'command' ? '...' : ''} onClick={() => setRecording('command')} className={`${inputCls} cursor-pointer text-center ${recording === 'command' ? 'border-gray-500/50 bg-gray-500/10' : ''}`} />
              </div>
              <button onClick={handleAddHotkey} disabled={!recordedKey} className="px-3 py-1.5 rounded-md text-xs font-semibold bg-gray-500/10 hover:bg-gray-500/20 border border-gray-500/20 text-gray-300 disabled:opacity-30">Add</button>
            </div>
              ...
              <div className="space-y-1 pt-1">
                {Object.entries(commandHotkeys).map(([key, command]) => (
                  <div key={key} className="flex items-center justify-between px-2 py-1 bg-gray-900/40 rounded">
                    <div className="flex items-center gap-2">
                      <span className="px-1.5 py-0.5 bg-gray-500/20 text-gray-300 text-[9px] font-mono font-bold rounded">{key.toUpperCase()}</span>
            ...
            <div className="space-y-2 p-2 bg-gray-900/30 rounded-md border border-gray-800/30">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={addServoToList} className="text-[9px] text-gray-400 hover:text-gray-300">+ Add Servo</button>
                </div>
                {servoList.map((s, i) => (
    ...
    <div className="p-4 space-y-3">
      ...
      <div className="bg-gray-800/40 rounded-lg border border-gray-800/50 overflow-hidden">
        <button onClick={() => setMotorsExpanded(!motorsExpanded)} className="w-full flex items-center justify-between p-3 hover:bg-gray-800/20 transition-colors">
          <div className="flex items-center gap-1.5"><Cog size={11} className="text-gray-500" /><span className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">Motors & Servos</span></div>
        ...
        {motorsExpanded && (
          <div className="p-3 pt-0 space-y-3">
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-1.5 cursor-pointer"><input type="checkbox" checked={allMotors} onChange={(e) => setAllMotors(e.target.checked)} className="w-3.5 h-3.5 rounded border-gray-600 bg-gray-800 text-gray-500" /><span className="text-[11px] text-gray-400">All motors</span></label>
              ...
              <div className="flex items-center justify-between"><span className="text-[10px] text-gray-500">Throttle</span><span className="text-[10px] font-mono text-gray-300">{throttle}%</span></div>
              <input type="range" min={1} max={100} value={throttle} onChange={(e) => setThrottle(parseInt(e.target.value))} className="w-full h-1.5 bg-gray-700 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-gray-400 [&::-webkit-slider-thumb]:rounded-full" />
            ...
            <div className="border-t border-gray-800/50 pt-3 space-y-2">
              ...
              <div className="flex items-center justify-between"><span className="text-[10px] text-gray-500">PWM</span><span className="text-[10px] font-mono text-gray-300">{pwm}</span></div>
              <input type="range" min={800} max={2200} value={pwm} onChange={(e) => setPwm(parseInt(e.target.value))} className="w-full h-1.5 bg-gray-700 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-gray-400 [&::-webkit-slider-thumb]:rounded-full" />
              ...
              <button onClick={testServo} className="w-full py-2 rounded-md text-xs font-semibold bg-gray-500/10 hover:bg-gray-500/20 border border-gray-500/20 text-gray-300">Set Servo</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="flex flex-col h-full">
      <div className="flex border-b border-gray-800/50 shrink-0">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1 px-3 py-2 text-[9px] font-semibold uppercase tracking-wider transition-colors ${subSection === 'params' ? 'text-gray-400 bg-gray-500/5' : 'text-gray-500 hover:text-gray-300'}`}
        >
          <SlidersHorizontal size={10} /> Params
        </button>
        <button
          onClick={() => setSubSection('mavlink')}
          className={`flex-1 flex items-center justify-center gap-1 px-3 py-2 text-[9px] font-semibold uppercase tracking-wider transition-colors ${subSection === 'mavlink' ? 'text-gray-400 bg-gray-500/5' : 'text-gray-500 hover:text-gray-300'}`}
        >
      ...
      <div className="bg-gray-800/40 rounded-lg border border-gray-800/50 p-3 space-y-3">
        ...
        {cacheStats !== null && (
          <div className="text-[11px] text-gray-300">
            <span className="font-mono text-gray-300">{cacheStats.toLocaleString()}</span> tiles cached
          </div>
        )}
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md text-xs font-semibold bg-gray-500/10 hover:bg-gray-500/20 border border-gray-500/20 text-gray-300 disabled:opacity-30 transition-colors"
          >
            <Download size={12} />
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex border-b border-gray-800/50 shrink-0 overflow-x-auto">
        {tabs.map((tab) => (
          <button key={tab.id} onClick={() => setSubTab(tab.id)} className={`flex-1 flex items-center justify-center gap-1 px-1.5 py-2 text-[9px] font-semibold uppercase tracking-wider transition-colors whitespace-nowrap min-w-0 ${subTab === tab.id ? 'text-gray-400 bg-gray-500/5' : 'text-gray-500 hover:text-gray-300'}`}>
